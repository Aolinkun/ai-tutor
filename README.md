...
```
你：我想学会财务报表分析
AI：先给一个诊断任务判断起点 → 讲解缺口 → 练习 → 新题检测 → 
    证据反馈 → 24 小时后复习 → 新场景迁移 → 掌握
```

**场景 2：解决问题**
```
你：客户要含税价，我不会算
AI：定位缺口（税率概念）→ 补最少知识 → 回到原问题 → 验证结果
```

**场景 3：复习**
```
你：复习上次学的
AI：读取课程状态 → 间隔检测（新题）→ 更新保持/迁移证据
```
...
AI：含税价 = 不含税价 × (1 + 税率)
    所以：不含税价 = 含税价 / (1 + 税率) = 1000 / 1.13 ≈ 884.96
    
    现在回到你的报价问题，客户要含税价 1000，你应该报不含税价 884.96。
    
    练习：如果客户要含税价 5000，不含税价是多少？
...
AI：[记录：solve 模式，补充了「含税价倒算」知识，验证了应用]
```
