...
```
1️⃣ 学会一个东西    → 我想看懂财报 / 学会做自媒体 / 理解行为经济学
2️⃣ 解决具体问题    → 分析这家公司的商业模式 / 我的账号没流量怎么办
3️⃣ 复习与检测      → 复习上周学的边际效应 / 检测一下我是否真的理解了
```
...
```
你：我想看懂上市公司财报
AI：先给一个诊断任务判断起点 → 讲解三张表的关系 → 分析真实财报 → 
    新公司检测 → 24 小时后换一家公司复习 → 不同行业迁移 → 掌握
```

**场景 2：解决问题**
```
你：我不理解为什么腾讯利润这么高，现金流却不多
AI：定位缺口（利润与现金流的区别）→ 讲解应计制与权责发生制 → 
    回到腾讯案例 → 分析具体科目 → 理解
```
...
```
你：复习上次学的沉没成本
AI：读取课程状态 → 给一个新的决策场景 → 检测是否能识别并应用 → 更新证据
```
...
**第 1 轮：诊断起点**
```
AI：某公司营业收入 100 亿，净利润 10 亿，经营活动现金流 8 亿。
    这三个数字分别代表什么？为什么数值不一样？

你：收入是赚的钱，利润是扣掉成本后赚的，现金流是实际收到的钱

AI：✅ 基本概念正确。核心在于「权责发生制」与「收付实现制」的区别：
    利润按权责发生制（该算的都算），现金流按实际收付。
    
    下一题（无提示）：公司赊销 1000 万商品，成本 600 万，还没收到钱。
    这笔交易如何影响利润表和现金流量表？
```
...
AI：❌ 利润应该是收入减成本。
    ...
    让我们分解：
    - 利润表：收入 +1000 万，成本 -600 万 → 利润 +400 万
    - 现金流量表：还没收到钱 → 经营现金流 0
    
    新题：公司收到客户预付款 500 万，商品还没发货。
    这笔交易如何影响两张表？
```

**第 3 轮：独立通过**
```
你：利润表不动（还没确认收入），现金流量表 +500 万

AI：✅ 完全正确！你理解了权责发生制的核心。当前单元达标（provisional）。
    
    24 小时后我们看一家真实公司的财报，检测保持。
...
**24 小时后：保持检测**
```
AI：这是某零售公司的财报片段，解释为什么它的经营现金流高于净利润。
    [给出真实数据]

你：[独立分析：应收账款减少、存货周转快、预收款增加...]

AI：✅ 延迟保持验证（retained）。
    下次我们换一个制造业公司，检测能否在不同行业迁移这个分析能力。
```

### 实例 2：解决自媒体账号问题

```
你：我做了 2 个月自媒体，发了 30 条内容，但播放量一直很低，不知道问题在哪

AI：[定位缺口] 你的内容选题是怎么确定的？有分析过哪些内容表现好吗？

你：我就是想到什么发什么，没太关注数据

AI：找到核心问题了。自媒体需要「数据驱动」而不是「感觉驱动」。
    ...
    现在回到你的账号：去后台看最近 10 条内容的完播率，
    找出最高的 2 条，分析它们的共同特征是什么。

你：[分析后] 发现讲具体案例的完播率高，纯讲理论的很低

AI：✅ 这就是第一个数据洞察！
    
    练习：基于这个发现，设计下一条内容的选题和结构。
...
AI：[记录：solve 模式，补充了「数据驱动选题」知识，验证了应用]
```
