# AI Tutor · 苏格拉底学习导师

基于 Benjamin Bloom 的 **Two Sigma 掌握学习法**，让 Claude Code / OpenClaw 变成你的专属一对一导师。

AI 不给你讲解——而是主动向你提问，根据你的回答实时调整难度。学习进度存成文件，跨对话不丢失。

---

## 一键安装

```bash
curl -fsSL https://raw.githubusercontent.com/Aolinkun/ai-tutor/main/install.sh | bash
```

运行后会提示选择安装目标：

```
请选择安装目标：
  1) Claude Code  (~/.claude/skills/)
  2) OpenClaw     (~/.openclaw/skills/)
  3) 两个都装
```

需要：macOS / Linux，以及 Claude Code 或 OpenClaw 其中之一

---

## 使用方法

在 Claude Code 或 OpenClaw 中直接说：

```
我想学维特根斯坦的逻辑哲学论
我想学 Rust 的所有权系统
我想学抖音运营       ← 会先问你想解决什么具体问题
继续学维特根斯坦     ← 从上次进度继续
```

---

## 核心机制

研究表明：一对一辅导 + 掌握学习法，可让学生超越 98% 的同龄人（Bloom, 1984）。

- **先澄清主题** — 「我想学抖音」→ 先问你想解决什么问题 → 收窄成精准主题
- **诊断再教学** — 2-3 个问题找到你真实的知识边界，不瞎教
- **掌握才推进** — 正确率 ≥ 80% 才进下一阶段，答错了退回巩固
- **三级提示** — 卡住说「提示」，分三级给线索，不直接给答案
- **情绪感知** — 连续答错时自动降难度、换方式，不机械扣分
- **每5单元复习** — 间隔重复，防止遗忘
- **上下文压缩** — 每3单元把关键信息存入文件，长对话不崩
- **跨主题关联** — 学新概念时主动关联你学过的其他主题

---

## 文件结构

安装位置：
```
~/.claude/skills/ai-tutor/        # Claude Code
~/.openclaw/skills/ai-tutor/      # OpenClaw
├── SKILL.md
└── references/
    ├── theory.md
    └── difficulty-levels.md
```

学习进度（存在你的项目目录）：
```
./learning/
└── [主题名称]/
    ├── progress.md
    ├── unit-01-[标题].md
    ├── unit-02-[标题].md
    └── summary.md              ← 结课后自动生成
```

---

## 版本记录

当前版本：**v1.3.0**

| 版本 | 更新内容 |
|------|---------|
| v1.3.0 | 新增主题澄清步骤；安装脚本支持 Claude Code / OpenClaw 选择 |
| v1.2.0 | 三级提示、四种纠错方式、情绪感知、上下文压缩、跨主题关联 |
| v1.1.0 | 续学机制、间隔复习、结课判断 |
| v1.0.0 | 初始版本 |

验证已安装版本：
```bash
grep "^# Version" ~/.claude/skills/ai-tutor/SKILL.md
# 或 OpenClaw：
grep "^# Version" ~/.openclaw/skills/ai-tutor/SKILL.md
```

---

## License

MIT
